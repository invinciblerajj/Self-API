...
app.get("/jokes/:id", (req,res)=>{
  ...
  const joke= jokes.find((joke)=> joke.id===result);
  res.json(joke);
})
...
app.get("/filter",(req,res)=>{
  ...
  const joke= jokes.find((joke)=> joke.jokeType===type);
  res.json(joke);
...
app.put("/jokes/:id",(req,res)=>{
  ...
  const curr= jokes.find((joke)=> joke.id===id);
...
app.delete("/jokes/:id",(req,res)=>{
  const id= parseInt(req.params.id);
  const curr= jokes.findIndex((joke)=> joke.id===id);
  if(curr>-1){
    jokes.splice(curr,1);
    res.sendStatus(200);
  }else{
...
app.delete("/all",(req,res)=>{
  ...
  if(key===masterKey){
    ...
    res.status(400).send("Not allowed to delet all");
  }
})
